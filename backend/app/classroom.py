...
@router.post(
    "/", summary="Google Classroomへアナウンスを投稿", response_description="投稿結果"
)
async def post_announcement(req: ClassroomRequest, creds=Depends(get_credentials)):
    """
    ...
    try:
        ...
        post = (
            service.courses()
            .announcements()
            .create(courseId=req.course_id, body=announcement)
            .execute()
        )
        return {
            "status": "success",
