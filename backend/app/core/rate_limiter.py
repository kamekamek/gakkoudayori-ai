...
class InMemoryRateLimiter:
    ...
    def is_allowed(self, identifier: str, limit: int, window: int) -> tuple[bool, Dict[str, any]]:
        """
        レート制限チェック
        ...
        if len(request_times) >= limit:
            ...
            if len(request_times) >= limit * 2:
                ...
                self.logger.warning(f"IP {identifier} blocked for 5 minutes due to excessive requests")
...
class APISecurityMonitor:
    ...
    def analyze_request(self, request: Request) -> Dict[str, any]:
        ...
        if request.url.query:
            ...
            for pattern_type in self.suspicious_patterns:
                query_threats.extend(self._check_patterns(request.url.query, pattern_type))
    ...
    def _is_suspicious_user_agent(self, user_agent: str) -> bool:
        ...
        suspicious_patterns = [
            ...
            "openvas", "burp", "metasploit", "w3af", "skipfish"
        ]
...
def get_security_report() -> Dict[str, any]:
    """セキュリティレポートを取得"""
    total_threats = sum(len(history) for history in security_monitor.threat_history.values())
    high_risk_ips = [
        ip for ip, score in security_monitor.threat_scores.items() 
        if score > 50
    ]
    ...
    return {
        ...
        "recent_attacks": sorted(recent_attacks, key=lambda x: x["timestamp"], reverse=True)[:10],
        "timestamp": datetime.utcnow().isoformat()
    }