...
class LayoutAgent(LlmAgent):
    ...
    def _extract_basic_info_from_conversation(self, conversation_text: str) -> dict:
        ...
        basic_info = {
            ...
            'teacher_name': '担任',
            'title': '学級通信',
            'content': conversation_text[:500],  # 会話の最初の500文字
...
<body>
    <div class="container">
        ...
        <div class="footer">
            <p>{basic_info['school_name']} {basic_info['grade']}</p>
        </div>
    </div>
</body>
</html>'''
...
HTMLのみを出力し、説明文は一切不要です。
            ...
            # LLMイベントからHTMLを保存（ADK推奨パターン）
            llm_html_valid = False
            async for save_event in self._save_html_from_llm_events(ctx, llm_events):
                yield save_event
                llm_html_valid = True
            ...
            if not is_consistent and json_obj:
                ...
                async for template_event in self._generate_html_from_template(ctx, json_obj):
    ...
    async def _save_html_from_llm_events(self, ctx: InvocationContext, llm_events) -> AsyncGenerator[Event, None]:
        """LLMイベントからHTMLを抽出してADK推奨パターンで保存"""
        try:
            # イベントからテキストを結合
            llm_response_text = ""
            ...
            # ADK推奨パターン: state_deltaでHTML状態を更新
            state_delta = {
                "html": html_content,
                "html_generated": True,
                "html_generation_timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                "html_length": len(html_content)
            }
            
            # EventActionsでstate_deltaを設定
            event_actions = EventActions(state_delta=state_delta)
            
            # 状態更新イベントをyield
            yield Event(
                author=self.name,
                content=Content(parts=[Part(text="HTMLレイアウトを生成しました。")]),
                actions=event_actions
            )
            
            logger.info("✅ ADK推奨パターンでHTML状態更新イベントを送信")
    ...
    async def _generate_html_from_template(self, ctx: InvocationContext, json_obj) -> AsyncGenerator[Event, None]:
        """JSONデータからテンプレートベースでHTMLを確実に生成（ADK推奨パターン）"""
        try:
...
{main_content}
    ...
    <div class="footer">
      <p>{school_name} {grade}</p>
            ...
            # ADK推奨パターン: state_deltaでテンプレートHTML状態を更新
            state_delta = {
                "html": template_html,
                "html_generated": True,
                "html_generation_timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            ...
            yield Event(
                author=self.name,
                content=Content(parts=[Part(text="テンプレートベースでHTMLを生成しました。")]),
                actions=event_actions
            )
            
            logger.info("✅ ADK推奨パターンでテンプレートHTML状態更新イベントを送信")
            ...
            yield Event(
                author=self.name,
                content=Content(parts=[Part(text="HTMLテンプレート生成中にエラーが発生しました。")]),
            )
    ...
    async def _get_json_from_adk_output_key(self, ctx: InvocationContext) -> str:
        """ADK公式パターンでセッション状態からJSONを取得（強化版）"""
        try:
            ...
            for key in json_keys_priority:
                ...
                if key not in ctx.session.state:
                    logger.info(f"❌ {key} キーがセッション状態に存在しません")
                    continue
                ...
                # データの空チェック
                if not json_data:
                    logger.info(f"❌ {key} キーのデータが空です")
                    continue
                    
                logger.info(f"✅ {key} キーからデータ取得: {len(str(json_data))} 文字")
                ...
                try:
                    ...
                    if missing_fields:
                        logger.warning(f"❌ {key} キーのJSONに必須フィールドが不足: {missing_fields}")
                        continue
                    ...
                    logger.info(f"✅ {key} JSONデータ検証成功: {school_name} {grade} 発行者:{author_name}")
                    
                    # サンプルデータでないことを確認
                    if '○○' in school_name or 'ERROR' in school_name or 'サンプル' in school_name:
                        logger.warning(f"❌ {key} キーのデータがサンプルデータです: school_name={school_name}")
                        continue
                    
                    return str(json_data)
                    
                except Exception as parse_error:
                    logger.warning(f"❌ {key} キーのJSON解析エラー: {parse_error}")
                    logger.warning(f"問題のあるデータ: {str(json_data)[:100]}...")
                    continue
